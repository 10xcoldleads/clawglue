...
if command -v xfce4-terminal &>/dev/null; then
    ...
    exec gnome-terminal --title="ClawGlue Security Check" -- /usr/local/bin/security-check
elif command -v x-terminal-emulator &>/dev/null; then
    exec x-terminal-emulator -e /usr/local/bin/security-check
else
    exec xterm -title "ClawGlue Security Check" -e /usr/local/bin/security-check
fi
"""
...
"""
    ...
    def _get_repo_branch(self):
        ...
        env_branch = os.environ.get("CLAWGLUE_BRANCH", "")
...
"""
    ...
    def run_setup(self):
        ...
        print("=" * 60)
        print("    ClawGlue Local Ubuntu 24.04 Setup")
        print("=" * 60)
        print(f"{Colors.ENDC}")
